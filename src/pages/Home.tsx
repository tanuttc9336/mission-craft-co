...
export default function Home() {
  ...
  return (
    <>
      <Helmet>
        <title>Undercat Creatives — Video Production & Creative Agency Bangkok</title>
        <meta name="description" content="Bangkok-based video production studio and creative agency. Brand films, social content, creative direction. Trusted by Audi Thailand, Greenline Golf. Starting at 49,000 THB." />
      ...
      >
        ...
        >
          ...
          {/* Center nav */}
          <nav className="pointer-events-auto hidden md:flex items-center gap-0 text-[10px] tracking-[0.24em] uppercase font-mono">
            {[
              { to: '/', label: 'Home', idx: '01' },
              { to: '/work', label: 'Work', idx: '02' },
              { to: '/golf', label: 'Golf', idx: '—' },
              {
                to: '/services',
                label: 'Services',
                idx: '—',
                children: [{ to: '/drone', label: 'Drone' }],
              },
              { to: '/credentials', label: 'Credentials', idx: '03' },
              { to: '/briefing-room', label: 'Briefing', idx: '—' },
    ...
    >
      ...
      >
        <span className="flex items-baseline gap-1.5">
          <motion.span
            animate={{ opacity: hover ? 0.9 : 0.35 }}
            transition={{ duration: 0.4, ease: EASE }}
            className="text-[8px] tracking-[0.2em] text-bone/40 tabular-nums"
          >
            {item.idx}
          </motion.span>
