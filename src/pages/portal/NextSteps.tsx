...
export default function NextSteps() {
  ...
  const { project, nextSteps, loading } = useProjectData(activeProjectId);
  ...
  const clientSteps = nextSteps.filter(s => s.owner === 'client');
  const undercatSteps = nextSteps.filter(s => s.owner === 'undercat');

  const StepList = ({ steps, label }: { steps: typeof nextSteps; label: string }) => (
    <div className="space-y-3">
      ...
      {steps.length === 0 ? (
        ...
        steps.map((step, i) => (
          <motion.div key={step.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="p-4 border border-border bg-card flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium">{step.title}</p>
              {step.notes && <p className="text-xs text-muted-foreground mt-1">{step.notes}</p>}
              <p className="text-[10px] text-muted-foreground mt-1">Due: {step.due_date}</p>
            </div>
