...
export default function Reviews() {
  ...
  const { project, reviews, loading } = useProjectData(activeProjectId);
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-3">
          {reviews.map((item, i) => {
            ...
            return (
              <motion.div key={item.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="border border-border bg-card">
                <button onClick={() => setExpandedId(isOpen ? null : item.id)} className="w-full flex items-center justify-between p-5 text-left">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">{item.approval_type.replace(/-/g, ' ')} · Due: {item.due_date}</p>
                  </div>
                  ...
                  {isOpen && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                      <div className="px-5 pb-5 space-y-5 border-t border-border pt-5">
                        <a href={item.review_link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 border border-border text-xs tracking-wide uppercase hover:bg-secondary transition-colors">
                          <ExternalLink className="h-3 w-3" />
                          Open review file
                        </a>
                        {!item.feedback_submitted ? (
                          <FeedbackForm reviewItemId={item.id} onSubmit={(data) => { console.log('Feedback submitted:', { itemId: item.id, ...data }); }} />
                        ) : (
