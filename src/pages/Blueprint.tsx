...
import { useSubmitBrief } from '@/hooks/useSubmitBrief';
import { Button } from '@/components/ui/button';
import { trackEvent } from '@/utils/analytics';
import { motion, AnimatePresence } from 'framer-motion';
import { Download, Copy, Phone, Lock, ArrowRight, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function Blueprint() {
  ...
  const handleUnlock = async () => {
    ...
    // Update local state first so context has final lead
    updateLead({ name: leadName, company: leadCompany, email: leadEmail, phone: leadPhone });
    trackEvent('blueprint_unlock', { email: leadEmail });

    // Build the brief snapshot with the just-captured lead (avoid stale state)
    const briefSnapshot = {
      ...brief,
      lead: {
        ...brief.lead,
    ...
    // Map entryPath from existing brief context to brief_requests enum.
    // ('fresh' / 'template' / 'existing-brief' → 'new' for now;
    //  client-type branch from B1 task will set 'audi'/'greenline'/etc.)
    const entryPath: 'new' | 'audi' | 'greenline' | 'driveline' | 'other-existing' = 'new';

    try {
      const result = await submitBrief.submit({
        brief: briefSnapshot,
        entryPath,
        locale: 'en', // i18n in Task #5 will source this from i18n context
      });
      trackEvent('brief_submitted', { briefId: result.id });
      setSubmittedId(result.id);
      setUnlocked(true);
      // Navigate to confirmation after a brief moment so user sees the unlock animation
      setTimeout(() => {
        navigate(`/confirmation/${result.id}`, {
          state: { briefSnapshot, blocks },
        });
      }, 1200);
    } catch (err) {
      // Error displayed inline below the form via submitBrief.error
  ...
  return (
    <div className="container py-16 md:py-24 max-w-3xl">
        ...
        {!unlocked && (
          ...
          >
            ...
            <Button
              ...
              disabled={!leadName.trim() || !leadEmail.trim() || submitBrief.loading}
            >
              {submitBrief.loading ? (
                <>Sending<span className="inline-block w-4 ml-2 animate-pulse">…</span></>
              ) : (
                <>Send Brief & Unlock <ArrowRight size={16} /></>
              )}
            </Button>
            ...
            {submitBrief.error && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="mt-4 flex items-start gap-2 p-3 border border-destructive/30 bg-destructive/5 text-sm"
              >
            ...
            <p className="text-[10px] text-muted-foreground mt-3">
              We'll review and reply within 24 hours. Your brief is encrypted in transit and stored on our private Supabase.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="no-print mt-12 flex flex-col sm:flex-row gap-4 items-center justify-center">
        {unlocked && submittedId ? (
          <>
            <motion.div
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              className="flex items-center gap-2 px-4 py-3 border border-foreground bg-secondary/50 text-sm"
            >
              <CheckCircle2 size={16} />
              <span>Brief sent. We'll reply within 24 hours.</span>
            </motion.div>
            <Button variant="hero" size="lg" asChild>
              <Link to={`/confirmation/${submittedId}`}>What happens next →</Link>
            </Button>
          </>
        ) : (
