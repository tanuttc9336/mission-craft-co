...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  // Editorial pages manage their own top bar / footer / background.
  const isEditorial =
    isHome ||
    location.pathname === '/work' ||
    location.pathname === '/golf' ||
    location.pathname === '/drone' ||
    location.pathname === '/services' ||
    location.pathname === '/credentials' ||
    location.pathname === '/briefing-room' ||
    /^\/work\/[^/]+$/.test(location.pathname);

  const linkCls = (active: boolean) =>
    isHome
      ? `font-mono text-[10px] tracking-[0.3em] uppercase transition-colors hover:text-white ${active ? 'text-white' : 'text-white/70'}`
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isEditorial ? 'bg-ink text-bone' : 'bg-background text-foreground'}`}>
      {/* Skip-to-content — keyboard-only, visible on focus */}
      ...
      >
        <div className="container flex items-center justify-between h-16">
          ...
          {/* Desktop Nav — consolidated to a real funnel:
              Work (cold) → Industries (warm specialty) → Briefing Room
              (intent) → Credentials (sales asset) → Contact (Pao).
              Home lives behind the logo. Services merged into Industries. */}
          <nav className="hidden md:flex items-center gap-8" aria-label="Primary">
            ...
            <Link to="/credentials" className={linkCls(isActive('/credentials'))}>Credentials</Link>
        ...
        <AnimatePresence>
          {mobileOpen && (
            ...
            >
              <nav className="container py-6 flex flex-col gap-1" aria-label="Mobile">
                ...
                <Link to="/credentials" onClick={() => setMobileOpen(false)} className={`${linkCls(isActive('/credentials'))} py-3`}>Credentials</Link>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground leading-relaxed mb-6">
                A small Bangkok studio that owns the whole thing — brief to final cut.
            ...
            <div>
              ...
              <div className="flex flex-col gap-2.5">
                ...
                <Link to="/golf" className="text-xs text-muted-foreground hover:text-foreground transition-colors">Golf</Link>
                <Link to="/services" className="text-xs text-muted-foreground hover:text-foreground transition-colors">Services</Link>
                <Link to="/drone" className="text-xs text-muted-foreground hover:text-foreground transition-colors pl-3">— Drone</Link>
              </div>
            </div>
